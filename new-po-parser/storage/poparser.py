...
def parse_comment(parse_state, unit):
    ...
    if len(next_line) > 0 and next_line[0] == '#':
        ...
        elif next_char == '~': # Special case: we refuse to parse obsoletes, since this should already be covered
            return None
        else:
            return None
...
def read_obsolete_lines(parse_state):
    obsolete_lines = []
    while parse_state.next_line.startswith('#~ '):
...
def parse_obsolete(parse_state, unit):
    ...
    if obsolete_lines == []:
        return None
    unit = parse_unit(parse_state.new_input(iter(obsolete_lines)))
    if unit is not None:
        unit.makeobsolete()
    return unit
